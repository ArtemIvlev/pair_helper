#!/usr/bin/env python3
"""
Скрипт для просмотра логов бэкенда Pair Helper
"""
...
def main():
    print("🔍 Детальный просмотр логов бэкенда Pair Helper...")
    print("=" * 60)
    ...
    # Получаем контейнеры
    print("3. Поиск контейнера бэкенда...")
    containers = get_containers(base_url, token, endpoint_id)
    if not containers:
        print("❌ Не удалось получить контейнеры")
        return
    
    # Ищем контейнер бэкенда
    backend_container = None
    for container in containers:
        names = container.get("Names", [])
        if any("pair-helper-backend" in name for name in names):
            backend_container = container
            break
    
    if not backend_container:
        print("❌ Контейнер pair-helper-backend не найден")
        return
    
    container_id = backend_container["Id"]
    container_name = backend_container["Names"][0] if backend_container["Names"] else "Unknown"
    container_status = backend_container.get("State", "Unknown")
    ...
    # Получаем логи
    print("4. Получение логов бэкенда...")
    print("-" * 60)
