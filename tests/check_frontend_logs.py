#!/usr/bin/env python3
"""
Скрипт для просмотра логов фронтенда Pair Helper
"""
...
def main():
    print("🎨 Детальный просмотр логов фронтенда Pair Helper...")
    print("=" * 60)
    ...
    # Получаем контейнеры
    print("3. Поиск контейнера фронтенда...")
    containers = get_containers(base_url, token, endpoint_id)
    if not containers:
        print("❌ Не удалось получить контейнеры")
        return
    
    # Ищем контейнер фронтенда
    frontend_container = None
    for container in containers:
        names = container.get("Names", [])
        if any("pair-helper-frontend" in name for name in names):
            frontend_container = container
            break
    
    if not frontend_container:
        print("❌ Контейнер pair-helper-frontend не найден")
        return
    
    container_id = frontend_container["Id"]
    container_name = frontend_container["Names"][0] if frontend_container["Names"] else "Unknown"
    container_status = frontend_container.get("State", "Unknown")
    ...
    # Получаем логи
    print("4. Получение логов фронтенда...")
    print("-" * 60)
