#!/usr/bin/env python3
"""
Скрипт для проверки пар в базе данных Pair Helper
"""
...
def main():
    print("🔍 Проверка пар в базе данных Pair Helper...")
    print("=" * 60)
    ...
    # Получаем контейнеры
    print("3. Поиск контейнера админки...")
    containers = get_containers(base_url, token, endpoint_id)
    if not containers:
        print("❌ Не удалось получить контейнеры")
        return
    
    # Ищем контейнер админки
    admin_container = None
    for container in containers:
        names = container.get("Names", [])
        if any("pair-helper-admin" in name for name in names):
            admin_container = container
            break
    
    if not admin_container:
        print("❌ Контейнер pair-helper-admin не найден")
        return
    
    container_id = admin_container["Id"]
    container_name = admin_container["Names"][0] if admin_container["Names"] else "Unknown"
    container_status = admin_container.get("State", "Unknown")
    ...
    # Получаем логи
    print("4. Получение логов админки...")
    print("-" * 60)
